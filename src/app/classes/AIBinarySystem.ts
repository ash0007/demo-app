 ...
 */
 ...
 import { Empty } from '../constants/empty-constants';
 ...
 class AIBInput {
     ...
     constructor(private _input1: Array<string> | number) {
         ...
         this.maxNumberOfOutputs = Math.pow(2, Math.pow(2, this.bits));
     }
 ...
 class AIBOutput {
     ...
     private getBinaryOutputSequence = (n: number, maxInputCombinations: number) => {
         ...
         bitSeq.unshift(...Array(maxInputCombinations - bitSeq.length).fill(0));
         return bitSeq;
     }
     calculate = (fnNo: number): void => {
         ...
         const minterm = this.outputSeq.map((value, index) => value ? index : -1).filter(v => (v !== -1));
         this.expression = (this.systemInputs.bits > 1 && this.systemInputs.bits < 5) ? ((this.outputSeq.indexOf(1) > -1) ? solve(this.systemInputs.symbols, minterm).expression : '0') : Empty.str;
     ...
     reset(): void {
         this.ioMap = null;
         this.outputSeq = [];
         this.outputString = Empty.str;
         this.expression = Empty.str;
         this.fn = Empty.str;
     }
     get = (): AIBOutput => {
         return {
             ...
             fn: this.fn,
             inputs: this.systemInputs.symbols
         } as AIBOutput;
     }
 }
 
 export class BooleanMagicBox {
     public systemWidth: number;
     private systemInputs: AIBInput;
     private systemOutput: AIBOutput;
     private _output: {[key: string]: AIBOutput};
     constructor(_systemWidth?: number);
     constructor(_systemWidth?: Array<string>);
     constructor(private _systemInputs?: AIBInput | number | Array<string>) {
        if(_systemInputs) {
            if(typeof _systemInputs === 'number') {
                this.systemInputs = new AIBInput(_systemInputs);
            } else if(Array.isArray(_systemInputs)) {
                this.systemInputs = new AIBInput(_systemInputs);
            } else {
                this.systemInputs = _systemInputs;
            }
        } else {
            this.systemInputs = new AIBInput(2);
        }
         this._output = {};
     }
     create(): void {
 ...
 class BinUtil {
     ...
     public static getStandardInputs(inputs: SystemInputType, isMinimalSystem?: boolean) {
         ...
         const convertTo2DArray = (isBinary: boolean) => {
             if(!isBinary) {
                 inputs = (inputs as []).map((e) => BinUtil.decimalToBitArray(e));
             } else {
                 inputs = (inputs as []).map((e) => BinUtil.binaryToBitArray(e));
             }
         }
         if(!BinUtil.isArrayOfArray(inputs) && !Array.isArray(inputs)) {
             isBinaryInput = BinUtil.isBinary(inputs);
             inputs = inputs.toString().split(",");
             convertTo2DArray(isBinaryInput);
         } else if(!BinUtil.isArrayOfArray(inputs) && Array.isArray(inputs)) {
             isBinaryInput = BinUtil.isBinary(inputs[0]);
             convertTo2DArray(isBinaryInput);
         } else {
             inputs = BinUtil.sanitizeTwoDArray(inputs as ArrayOfArray<string | number>);
         }
         
         return isMinimalSystem ? BinUtil.minStandardInputsArray(inputs as ArrayOfArray<string | number>) : BinUtil.maxStandardInputsArray(inputs as ArrayOfArray<string | number>);
     }
